...
const SignUpForm = () => {
    ...
    const handleSignup = (e) => {
        ...
        if (!emailRegex.test(email))
            return setEmailInvalid("L'adresse e-mail est invalide");
        ...
        axios({
            ...
            .catch((err) => {
                if (err.response.status === 400)
                    return setPasswordInvalid(err.response.data.message);
                if (err.response.status === 500)
                    return setEmailInvalid("L'adresse e-mail existe déjà");
            });
    };
